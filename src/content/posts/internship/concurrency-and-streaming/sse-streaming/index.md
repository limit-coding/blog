...
3. 常见追问点：如果 Agent 中途要执行 Tool Call，流会不会中断？后端怎么在流里插入"工具调用中"的中间状态事件？多个 SSE 连接并发时，Nginx/网关的 `proxy_buffering` 必须关掉，否则会导致假流式——数据攒够了才一次性发出去，详见 [Nginx 反向代理实战](/internship/backend-networking/nginx-fundamentals) 里的踩坑记录。
