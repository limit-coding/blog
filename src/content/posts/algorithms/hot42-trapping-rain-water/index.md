...
第一反应是这题跟 [LC11 盛最多水的容器](/algorithms/hot11-container-with-most-water)很像，也想用双指针两侧逼近。但很快卡住了：LC11 只关心两个端点围出的最大面积，而接雨水要算的是**每一个位置**能存多少水，判定条件是"左边最高柱子和右边最高柱子中较矮的那个，减去当前柱子的高度"——这依赖的是左右两侧各自的最大值，不是简单的两端收缩能覆盖的。
